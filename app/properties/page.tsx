...
export default function AdminPropertyPage() {
  ...
  const createProperty = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data) {
        // uuid があればそれ、なければ id を使う（undefined回避）
        const pathId = data.uuid || data.id;
        setGeneratedUrl(`${window.location.origin}/${pathId}`);
        setName(''); 
        setAddress('');
      }
    } catch (err: any) {
  ...
  return (
    <div className="max-w-md mx-auto p-8 bg-white min-h-screen">
      <h1 className="text-2xl font-bold mb-6 text-gray-800 border-b pb-2">🏢 物件登録</h1>
      <form onSubmit={createProperty} className="space-y-4 mb-8">
        <div>
          <label className="text-sm font-medium text-gray-600">物件名</label>
          <input 
            className="w-full border p-3 rounded mt-1 bg-gray-50 outline-none"
        ...
        >
          {loading ? '登録中...' : '物件を登録してURL発行'}
        </button>
      </form>

      {generatedUrl && (
        <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-blue-700 font-bold mb-2">✅ 住民用URL：</p>
          <input 
            readOnly 
          ...
          />
          <p className="text-[10px] text-gray-400 mt-2">※このURLを住民に配布してください</p>
        </div>
      )}
    </div>
  );
