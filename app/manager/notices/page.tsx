...
export default function ManagementNoticePage() {
  ...
  const [scheduledAt, setScheduledAt] = useState(''); // 予約配信日時
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-10 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-10 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
          ...
          <div className="w-full lg:w-96 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          {/* メイン：投稿フォーム */}
          <div className="flex-1">
            <form className="bg-white rounded-[3.5rem] p-8 md:p-14 shadow-2xl shadow-slate-200/40 border border-slate-100 space-y-10">
              
              <div className="flex justify-between items-center border-b border-slate-50 pb-8">
                <div className="flex bg-slate-100 p-1 rounded-2xl">
                  {['published', 'scheduled', 'draft'].map((s) => (
                    <button key={s} type="button" onClick={() => setStatus(s as any)}
                      className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all ${status === s ? 'bg-white shadow-md text-blue-600' : 'text-slate-400'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[{ id: 'urgent', label: '緊急連絡', icon: '🚨' }, { id: 'maintenance', label: '工事・点検', icon: '🔧' }, { id: 'campaign', label: 'お知らせ', icon: '📢' }, { id: 'local', label: '地域情報', icon: '📍' }].map((cat) => (
                      <button key={cat.id} type="button" onClick={() => setCategory(cat.id)}
                        className={`flex items-center gap-3 p-4 rounded-2xl text-[11px] font-bold border-2 transition-all ${category === cat.id ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-slate-100 text-slate-400'}`}>
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-1">スケジュール</label>
                  <div className="bg-slate-50 p-6 rounded-[2rem] space-y-4">
                    {status === 'scheduled' && (
                      <div className="space-y-1">
                        <span className="text-[9px] font-bold text-blue-600 ml-1">配信予定日時</span>
                        <input type="datetime-local" className="w-full p-3 rounded-xl border-none font-bold text-sm" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)} />
                      </div>
                    )}
                    ...
                    {!isPermanent && <input type="datetime-local" className="w-full p-4 rounded-xl border-none font-bold text-sm" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)} />}
                  </div>
                </div>
              </div>
                  ...
                  <label className={`flex flex-col items-center justify-center border-2 border-dashed rounded-[2.5rem] h-64 cursor-pointer transition-all ${pdfUrl ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-200 hover:border-blue-300'}`}>
                    {uploading ? <div className="animate-spin h-8 w-8 border-b-2 border-blue-600 rounded-full" /> : 
                      <div className="text-center p-6">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{pdfUrl ? '完了' : 'ファイルを添付'}</p>
                      </div>
                    }
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 flex items-center gap-4 p-6 bg-blue-50 rounded-[2.5rem] border border-blue-100 group cursor-pointer">
                  ...
                  <label htmlFor="push-notify" className="flex-1 cursor-pointer">
                    ...
                    <p className="text-[10px] text-blue-600 font-bold opacity-70">住民のスマホへ通知を飛ばします</p>
                  </label>
                </div>

                <button onClick={(e) => handleSubmit(e)} disabled={isSubmitting} className="flex-[2] bg-blue-600 text-white py-8 rounded-[3rem] font-black text-2xl hover:bg-slate-900 transition-all shadow-2xl shadow-blue-200 disabled:opacity-50 uppercase tracking-tighter italic">
                  {isSubmitting ? '処理中...' : status === 'draft' ? '下書き保存する' : 'この内容で配信設定'}
                </button>
              </div>
            </form>
          </div>
                  ...
                  return (
                    <div key={notice.id} className="group border-b border-slate-50 pb-8 last:border-0">
                      <div className="flex gap-4 items-start mb-5">
                        <span className="text-lg bg-slate-50 w-10 h-10 rounded-xl flex items-center justify-center group-hover:bg-blue-50">{notice.category === 'urgent' ? '🚨' : '📢'}</span>
                        <div className="flex-1">
                      ...
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 cursor-pointer hover:bg-blue-50 transition-all" onClick={() => handleShowReadDetails(notice.id)}>
                        <div className="flex justify-between items-end mb-2">
                          <span className="text-[9px] font-black text-slate-400 italic">閲覧状況（詳細表示）</span>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-6" onClick={() => setShowReadList({ ...showReadList, show: false })}>
            <div className="bg-white w-full max-w-md rounded-[3rem] p-10 space-y-6" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center">
                <h4 className="text-xl font-black italic">既読ユーザー一覧</h4>
              ...
              <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
                ...
                )) : <p className="text-center text-slate-400 py-10">既読データはありません</p>}
              </div>
            </div>
          </div>
        )}
            ...
            <div className="relative">
              ...
              <div className="w-[320px] h-[640px] bg-white rounded-[3rem] overflow-hidden border-[8px] border-slate-800 shadow-2xl flex flex-col">
                <div className="bg-blue-600 p-6 pt-12 text-white">
                  <div className="text-[9px] font-black opacity-60 uppercase tracking-widest mb-1">Preview Mode</div>
                  <h5 className="font-black text-lg leading-tight">{title || '（タイトルなし）'}</h5>
                </div>
                <div className="p-6 flex-1 space-y-4">
                  <div className="flex gap-2">
                    <span className="bg-blue-100 text-blue-600 px-3 py-1 rounded-full text-[9px] font-black uppercase">{category}</span>
                    <span className="bg-slate-100 text-slate-400 px-3 py-1 rounded-full text-[9px] font-black italic">TODAY</span>
                  </div>
                  <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">{content || '本文がここに入ります。'}</p>
                </div>
                <div className="p-6 border-t border-slate-50">
                  <div className="w-full bg-blue-600 h-12 rounded-2xl flex items-center justify-center text-white text-xs font-black">既読にする</div>
                </div>
              </div>
            </div>
          </div>
