...
export default function ManagementNoticePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const payload: any = {
          title: finalTitle, content, category, pdf_url: combinedPdfUrls, status,
        ...
        if (status === 'scheduled') payload.published_at = new Date(scheduledAt).toISOString();
        ...
        // 2. 🎯 メール通知APIの呼び出し（即時公開時のみ）
        if (status === 'published') {
          await fetch('/api/send-notification', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              propertyId: deliveryTarget === 'property' ? targetPropId : null,
              uploadedFiles: uploadedFiles, // 添付ファイル情報
              target: deliveryTarget
            }),
          });
        }

        alert(deliveryTarget === 'system' ? '全ユーザーへ配信とメール通知を行いました。' : '住民へ配信とメール通知を行いました。');
          ...
          <div className="flex-1">
            <form onSubmit={handleSubmit} className={`bg-white rounded-[4rem] p-8 md:p-14 shadow-2xl border-2 space-y-12 transition-all ${deliveryTarget === 'system' ? 'border-indigo-600 ring-4 ring-indigo-50' : 'border-slate-50'}`}>
              ...
              <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100">
                <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2">通知メール プレビュー</p>
                <div className="text-sm font-bold text-slate-700 leading-relaxed italic">
                  新しいぽすっとが届きました！！<br />
                  以下のURLからぽすっとを確認してください！<br />
                  <span className="text-blue-600 underline">https://posutto.vercel.app/login?type=user</span>
                </div>
              </div>
              ...
              <button type="submit" disabled={isSubmitting} 
                ...
                {isSubmitting ? 'SENDING...' : deliveryTarget === 'system' ? '📢 運営連絡（全ユーザー）へ配信・メール通知' : '住民へ配信・メール通知を実行'}
              </button>
            </form>
          </div>
