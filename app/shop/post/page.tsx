...
export default function ShopPostPage() {
  ...
  useEffect(() => {
    const initializePortal = async () => {
      try {
        ...
        // ロール確認（PGRST116エラー回避のためmaybeSingle）
  ...
  return (
    <AdminLayout userType={userRole === 'ADMIN' ? 'ADMIN' : 'SHOP'}>
      <div className="p-4 md:p-8 bg-[#F8FAFC] min-h-screen">
        
        {/* --- クイックナビゲーション（追加） --- */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            { label: '広告管理', icon: '📢', path: '/shop/ads', color: 'hover:border-orange-500' },
            { label: '分析レポート', icon: '📊', path: '/shop/analytics', color: 'hover:border-blue-500' },
            { label: '店舗設定', icon: '⚙️', path: '/shop/settings', color: 'hover:border-slate-900' },
            { label: '新規作成', icon: '➕', path: '/shop/post', color: 'hover:border-green-500' },
          ].map((nav, i) => (
            <button 
              key={i}
              onClick={() => router.push(nav.path)}
              className={`bg-white border border-slate-100 p-6 rounded-[2rem] flex flex-col items-center gap-2 transition-all shadow-sm ${nav.color} hover:shadow-md active:scale-95`}
            >
              <span className="text-xl">{nav.icon}</span>
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-tighter">{nav.label}</span>
            </button>
          ))}
        </div>
            ...
            <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-xl border border-slate-100">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 tracking-tighter italic uppercase">Posutto <span className="text-orange-500">Post</span></h1>
                  </div>
                  <p className="text-slate-400 text-[10px] font-bold tracking-[0.3em] uppercase">Digital Local Ads Distribution</p>
                </div>
                <button 
                  type="button"
                  onClick={handleAIGenerate} 
                  disabled={aiGenerating} 
                  className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-orange-600 transition shadow-xl active:scale-95 disabled:opacity-50"
                >
                  {aiGenerating ? 'AI生成中...' : '✨ 内容をAIで作成'}
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-orange-500 uppercase tracking-widest ml-1">本日の一言（広告のベース）</label>
                  <input 
                    ...
                    placeholder="例：本日限定！焼き立てパン10%OFF"
                  />
                </div>
                ...
                <div className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col md:flex-row md:items-center justify-between gap-8 shadow-2xl relative overflow-hidden">
                  <div className="relative z-10">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-4">配信エリア選択</label>
                  ...
                  <div className="relative z-10 text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase mb-1">現在の配信対象</p>
                    <p className="text-5xl font-black text-white tracking-tighter">
                      {nearbyProperties.length}
                      <span className="text-sm ml-2 text-orange-500 italic uppercase">Assets</span>
                    </p>
                  </div>
                ...
                {/* プレビュー入力 */}
                <div className="bg-white border-2 border-slate-100 rounded-[3rem] p-10 space-y-8 shadow-inner">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">広告タイトル</label>
                    <input className="w-full bg-transparent border-b-2 border-slate-100 p-2 text-2xl font-black outline-none focus:border-orange-500 transition-all placeholder:text-slate-200" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="AIボタンを押すか、直接入力してください" required />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">広告本文</label>
                    <textarea className="w-full bg-transparent p-2 text-md font-bold min-h-[120px] outline-none border-none resize-none leading-relaxed" value={content} onChange={(e) => setContent(e.target.value)} placeholder="特典の内容や、詳しい店舗情報を記入してください..." />
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">店舗表示名</label>
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">掲載期限</label>
                ...
                {/* アップロード */}
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">画像・チラシ（任意）</label>
                  <label className="w-full bg-slate-50 border-2 border-dashed border-slate-200 p-10 rounded-[2.5rem] cursor-pointer hover:bg-slate-100 hover:border-orange-500 transition-all flex flex-col items-center justify-center gap-4">
                    ...
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-sans">
                      {uploading ? 'Uploading...' : pdfUrl ? 'Ready to Send' : 'Click to Upload Image/PDF'}
                    </span>
                ...
                <button 
                  ...
                  className="w-full bg-slate-900 text-white py-8 rounded-[2.5rem] font-black shadow-2xl shadow-slate-200 hover:bg-orange-600 transition-all active:scale-[0.98] text-xl italic tracking-tighter uppercase"
                >
                  {isSubmitLoading ? 'Distributing...' : '近隣住民へポスティング！'}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="w-full lg:w-96">
            <div className="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100 sticky top-10">
              <div className="flex items-center gap-2 mb-10">
                ...
                <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 italic">Recent Distribution</h2>
              </div>
              ...
              <div className="space-y-8">
                {recentAds.map(ad => (
                  <div key={ad.id} className="group cursor-pointer" onClick={() => router.push(`/shop/analytics?id=${ad.id}`)}>
                    ...
                    <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-slate-400">
                      ...
                      <span className="bg-slate-50 px-2 py-1 rounded-md">R: {ad.radius_km}km</span>
                    </div>
                ...
                {recentAds.length === 0 && (
                  <div className="text-center py-10">
                    <p className="text-[10px] text-slate-300 font-black uppercase tracking-widest">No history available</p>
                  </div>
                )}
              </div>
              ...
              <button 
                ...
                className="w-full mt-12 py-4 border-2 border-slate-100 rounded-2xl text-[10px] font-black text-slate-400 hover:bg-slate-900 hover:text-white transition-all uppercase tracking-widest"
              >
                詳細レポート →
              </button>
            </div>
        ...
        <footer className="mt-12 text-[9px] text-slate-400 text-center font-bold uppercase tracking-[0.4em]">
          Posutto Shop Portal - Digital Advertising System
        </footer>
      </div>
    </AdminLayout>
  );
