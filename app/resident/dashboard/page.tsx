...
export default function ResidentDashboard() {
  ...
  const uiTexts: any = {
    ja: {
      ...
      adsTitle: '近鄰のトピックス', noAds: '周辺のお得な情報を探索中...', home: 'ホーム', settings: '設定', logout: 'ログアウト',
  ...
  const fetchResidentData = async () => {
    try {
      ...
      if (prof?.property_id) {
        ...
        // 🎯 通常掲示板には、管理会社通知と運営会社通知を結合してソート
        // 運営側通知には is_system_notice フラグを付与して区別
        const formattedSystemNotices = (systemNotices || []).map(sn => ({ ...sn, is_system_notice: true }));
        const combinedNotices = [...regularNotices, ...formattedSystemNotices].sort((a, b) => 
            new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
        );
  ...
  const handleAdInteraction = async (adId: string, pdfUrl: string) => {
    ...
    supabase.rpc('increment_ad_clicks', { target_ad_id: adId }).then(({ error }) => {
        if (error) console.warn("Click tracking skipped.");
    });
  ...
  return (
    <div className="max-w-md mx-auto bg-[#F4F7FA] min-h-screen pb-44 font-sans overflow-x-hidden relative">
      ...
      {zoomedImage && (
        <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex flex-col justify-center items-center p-4" onClick={() => setZoomedImage(null)}>
          <div className="absolute top-6 right-6 z-[110]">
            <button onClick={() => setZoomedImage(null)} className="w-12 h-12 bg-white/20 hover:bg-white/40 backdrop-blur-md rounded-full flex items-center justify-center text-white text-xl font-black">✕</button>
          </div>
          <div className="w-full h-full flex items-center justify-center overflow-auto" onClick={(e) => e.stopPropagation()}>
            <img src={zoomedImage} alt="Zoomed" className="max-w-full max-h-[85vh] object-contain rounded-2xl shadow-2xl animate-in zoom-in-90 duration-300"/>
          </div>
        </div>
      )}
      ...
      <div className="relative bg-slate-900 pt-16 pb-20 px-8 rounded-b-[4.5rem] shadow-2xl overflow-hidden">
        ...
        <div className="relative z-10 text-center">
            <p className="text-blue-400 text-[10px] font-black uppercase tracking-[0.3em] mb-1">{profile?.properties?.name || 'Smart Resident Portal'}</p>
      ...
      <div className="px-6 space-y-10 -mt-8 relative z-20">
          ...
          {translatedPriorities.length > 0 ? (
            <div className="space-y-6">
              {translatedPriorities.map((post) => {
                ...
                return (
                  <div key={post.id} className="bg-slate-900 rounded-[3.5rem] shadow-2xl border border-slate-800 p-10 text-white relative group transition-all">
                    ...
                    <div className="relative z-10">
                      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                        ...
                        <span className="text-[9px] font-bold text-white/40 tabular-nums">{t.postedAt}: {formatDate(post.published_at || post.created_at)}</span>
                      </div>
                      <h3 className="text-2xl font-black leading-tight mb-4 italic tracking-tight underline decoration-blue-500/50 underline-offset-4">{post.title}</h3>
                      <p className="text-[14px] text-slate-300 leading-relaxed font-medium bg-white/5 p-6 rounded-[2rem] border border-white/5 mb-8">{post.content}</p>
                      <div className="space-y-3">
                        <div className="grid grid-cols-1 gap-2">
                          {pdfs.length > 0 && pdfs.map((url, index) => (
                            <button key={index} onClick={() => handleAdInteraction(post.id, url)} className="w-full bg-white text-slate-900 py-4 px-6 rounded-2xl font-black text-[11px] flex items-center justify-between hover:bg-blue-50 transition-colors active:scale-[0.98]">
                              <div className="flex items-center gap-3"><span className="text-lg">📄</span><span className="uppercase tracking-tighter">{t.viewFlyer}</span></div>
          ...
          <div className="p-10">
            ...
            {translatedNotices.length > 0 ? (
              <div className="space-y-8 animate-in fade-in duration-1000">
                {translatedNotices.slice(0, 8).map((notice, i) => {
                    const pdfs = getPdfUrls(notice.pdf_url);
                    // 🎯 運営通知かどうかでスタイルを分岐
                    const isOfficial = notice.is_system_notice;
                        ...
                        <div key={notice.id} className={`${i !== 0 ? 'pt-8 border-t border-slate-50' : ''}`}>
                            <div className="flex justify-between items-start mb-3">
                                <div className="flex flex-col gap-2">
                                  {isOfficial && (
                                    <span className="w-fit text-[8px] font-black bg-slate-900 text-indigo-400 px-3 py-1 rounded-full uppercase tracking-widest shadow-sm">
                                      {t.officialBadge}
                                    </span>
                                  )}
                            ...
                            <div className={`text-[14px] leading-relaxed p-6 rounded-[2rem] whitespace-pre-wrap font-medium border mb-4 
                                ${isOfficial 
                                  ? 'bg-slate-900 text-slate-300 border-slate-800 shadow-inner' 
                                  : 'bg-[#F9FBFF] text-slate-600 border-slate-100'}`}
                            >
                                {notice.content}
                            </div>
                            {pdfs.length > 0 && (
                                <div className="flex flex-wrap gap-2 mt-2">
                                    {pdfs.map((url, idx) => (
                                        <button key={idx} onClick={() => handleAdInteraction(notice.id, url)} className="bg-white border border-slate-200 text-slate-600 px-4 py-2.5 rounded-xl text-[10px] font-black flex items-center gap-2 hover:bg-blue-50 transition-colors shadow-sm">
                                            📄 {t.viewFlyer}
                                        </button>
                                    ))}
                                </div>
              ...
              <div className="py-10 text-center">
                <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-100 shadow-inner"><span className="text-3xl grayscale opacity-20">📣</span></div>
        ...
        <section className="bg-white rounded-[3.5rem] overflow-hidden shadow-xl shadow-slate-200/50 border border-white">
          ...
          <div className="p-8 bg-slate-50/50 text-center border-b border-slate-100"><p className="text-[11px] font-black text-slate-500 leading-relaxed px-2">{t.trashHint}</p></div>
          <div className="p-6 bg-slate-50 flex justify-between items-center px-8">
            <input type="month" value={selectedYearMonth} onChange={(e) => setSelectedYearMonth(e.target.value)} className="bg-white px-5 py-2.5 rounded-2xl text-[10px] font-black text-slate-800 outline-none border border-slate-200 shadow-sm" />
            <button onClick={() => setIsEditingCalendar(!isEditingCalendar)} className={`text-[10px] font-black uppercase px-6 py-2.5 rounded-full transition-all shadow-sm ${isEditingCalendar ? 'bg-slate-900 text-white' : 'bg-white text-emerald-600 border border-emerald-100'}`}>{isEditingCalendar ? t.complete : t.set}</button>
          </div>
          <div className="p-6 min-h-[260px] flex items-center justify-center bg-white px-8">
            {garbageCalendars[selectedYearMonth] ? (
              <div className="w-full">
                <img src={garbageCalendars[selectedYearMonth]} alt="Calendar" className="w-full h-auto rounded-[2.5rem] shadow-md border border-slate-50 active:scale-95 transition-transform cursor-pointer" onClick={() => setZoomedImage(garbageCalendars[selectedYearMonth])} />
                <p className="mt-4 text-center text-[9px] font-black text-slate-300 uppercase tracking-widest">タップして拡大表示</p>
              </div>
            ) : (
              <div className="text-center py-10">
                <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6 border border-slate-100 shadow-inner"><span className="text-4xl opacity-20">📸</span></div>
