    ...
    <div className="max-w-md mx-auto bg-[#F8FAFC] min-h-screen pb-40 font-sans overflow-x-hidden">
      ...
      <div className="p-6 space-y-10 -mt-8">
        ...
        <section className="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-slate-200 border border-white">
          ...
          {/* 画像を常時表示するメインエリア */}
          <div className="mt-4 rounded-[2rem] overflow-hidden border-2 border-slate-100 bg-slate-50 min-h-[250px] flex items-center justify-center">
            {garbageCalendars[selectedMonth] ? (
              <img 
                src={garbageCalendars[selectedMonth]} 
                alt="ゴミカレンダー" 
                className="w-full h-auto object-contain shadow-inner cursor-pointer"
                onClick={() => window.open(garbageCalendars[selectedMonth], '_blank')}
              />
            ) : (
              <div className="text-center p-10">
                <p className="text-[11px] font-black text-slate-400 italic">カレンダーが登録されていません</p>
                <p className="text-[9px] text-slate-300 mt-2">下の「表登録」からアップロードしてください</p>
              </div>
            )}
          </div>
        </section>
